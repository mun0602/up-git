...
check_github_connection() {
    ...
    echo "Kiểm tra repository GitHub..."
    
    while [ $attempt -le $max_attempts ]; do
        if git ls-remote "$repo_url" &> /dev/null; then
            echo "Repository GitHub OK."
            return 0
        else
            echo "Lần thử $attempt/$max_attempts: Không thể kết nối đến repository."
    ...
    echo "Không thể kết nối đến repository sau $max_attempts lần thử."
...
while true; do
    ...
    # Kiểm tra repository GitHub trực tiếp
